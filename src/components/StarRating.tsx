...
const StarRating: React.FC<StarRatingProps> = ({ 
  ...
  // Ensure rating is a number or set to 0
  ...
  const handleStarClick = (newRating: number) => {
    if (interactive && onRatingChange) {
      // Log the clicked rating for debugging
  ...
  const renderStars = () => {
    ...
    const roundedRating = Math.round(activeRating * 2) / 2; // For half stars
  ...
  return (
    <div className={`flex items-center ${className}`} data-testid="star-rating">
      ...
      {showRating && (
        <div className="ml-2 text-sm font-medium text-gray-600 dark:text-gray-400" data-testid="rating-value">
