...
function FilterCategory({ query: { query, setQuery } }) {
  const [selectedCat, setSelectedCat] = useState(query.category || '0');
  const [priceRange, setPriceRange] = useState({
    min: query.minPrice || "",
    max: query.maxPrice || ""
  });
  const [priceError, setPriceError] = useState("");
  const [minRating, setMinRating] = useState(query.minRating ? parseFloat(query.minRating) : 0);
  ...
  useEffect(() => {
    startTransition(() => {
      ...
      if (query.minRating) {
        setMinRating(parseFloat(query.minRating));
      } else {
        setMinRating(0);
      }
  ...
  const handleRatingChange = (rating) => {
    ...
    startTransition(() => {
      const newQuery = { ...query, minRating: rating };
      setQuery(newQuery);
    });
  };
